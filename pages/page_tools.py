"""
Menu 3: 분석 & 관리 도구
F3: KOL 자동 발굴, A3: 예산 시뮬레이터, F5: 유가 포스팅 분석,
A4: 소재 분석, F6: 노출-쿼리 상관분석, KOL DB
"""
...
from db import (
    save_scored_df, get_all_kols, get_kol_history, get_kol_delta,
...
def render_tools_page(IS_CLOUD, num_posts):
    # 서브 메뉴
    tool_tabs = st.tabs([
        "🔍 KOL 발굴", "💰 예산 시뮬레이터", "📈 유가 포스팅 분석",
        "📊 노출-쿼리 분석", "📂 KOL DB"
    ])
    ...
    with tool_tabs[0]:
        ...
        if st.button("🚀 KOL 발굴 시작", type="primary", use_container_width=True, key="f3_run"):
            ...
            if not keywords or not f3_plat:
                ...
                for ci, cand in enumerate(candidates):
                    prog.progress(0.6+0.4*(ci+0.5)/max(len(candidates),1), text=f"[{ci+1}/{len(candidates)}] {cand['kol_name']}...")
                    pw_page = None
                    if pw_ctx:
                        try: pw_page = pw_ctx.new_page()
                        except: pass
                    raw = scrape_kol(url=cand["profile_url"], num_posts=12, pinned_ids=[], playwright_page=pw_page)
                    if pw_page:
                        try: pw_page.close()
                        except: pass
                    fee = float(f3_fee) if f3_fee else None
                    kol_list.append(KOLMetrics(
                        name=cand["kol_name"], platform=cand["platform"], url=cand["profile_url"],
                        fee=fee, avg_views=raw.get("avg_views"), avg_likes=raw.get("avg_likes"),
                ...
                if pw_ctx:
                    try: pw_ctx.close(); _pw.__exit__(None,None,None)
                    except: pass
        ...
        if st.session_state.get("f3_contact_list") is not None:
            ...
            cols = ["KOL명","플랫폼","평균 조회수","평균 좋아요","ER%","CPV(¥/회)","종합점수","등급","채택권고","URL"]
            cols = [c for c in cols if c in cdf.columns]
            st.dataframe(cdf[cols].sort_values("종합점수", ascending=False, na_position="last"),
            ...
            try:
                ...
                buf.seek(0)
                st.download_button("📥 컨택 리스트 Excel", data=buf.getvalue(),
    ...
    with tool_tabs[1]:
        ...
        a3_raw = get_all_kols()
        a3_eligible = [k for k in a3_raw if k.get("latest_fee") and k.get("latest_views")
                       and k["latest_fee"]>0 and k["latest_views"]>0]
        if not a3_eligible:
            st.info("DB에 비용+조회수 데이터가 있는 KOL이 없습니다.")
        else:
            ac1, ac2 = st.columns(2)
            with ac1:
                budget = st.number_input("총 예산 (JPY)", 10000, step=50000, value=500000, key="a3_budget")
            with ac2:
                goal = st.selectbox("최적화 목표", ["총 노출 최대화","총 인게이지먼트 최대화","CPV 최소화"], key="a3_goal")
            ...
            if st.button("🧮 최적 조합 계산", type="primary", use_container_width=True, key="a3_run"):
                ...
                for k in a3_eligible:
                    ...
                    eng = views*er/100
                    if goal=="총 노출 최대화": val = int(views)
                    elif goal=="총 인게이지먼트 최대화": val = int(eng)
                    else: val = int(views/fee) if fee>0 else 0
            ...
            if st.session_state.get("a3_result"):
                ...
                mc = st.columns(4)
                with mc[0]: st.metric("선택 KOL", f"{len(sel)}명")
                with mc[1]: st.metric("예상 비용", f"¥{int(tc):,}")
                with mc[2]: st.metric("예상 노출", f"{int(tv):,}")
                with mc[3]: st.metric("평균 CPV", f"¥{tc/tv:.2f}" if tv>0 else "—")

                rows = []
                for s in sel:
                    k=s["data"]; v=k.get("latest_views") or 0
                    rows.append({"KOL명":k["name"],"플랫폼":k["platform"],
                                 "비용":f"¥{int(k['latest_fee']):,}","예상 조회수":f"{int(v):,}",
                                 "점수":k.get("latest_score"),"등급":k.get("latest_grade","—"),
    ...
    with tool_tabs[2]:
            ...
            if campaigns:
                ...
                if "f5_rows" not in st.session_state:
                    st.session_state.f5_rows = pd.DataFrame({
                        "KOL명":[""],"URL":[""],"비용(JPY)":[0],"포스팅일":[""],"소재유형":[""]
                    })

                f5_edited = st.data_editor(
                    st.session_state.f5_rows, use_container_width=True, num_rows="dynamic", hide_index=True,
                    column_config={
                ...
                if not f5_valid.empty and st.button("🚀 등록 + 스크래핑", type="primary", use_container_width=True, key="f5_run"):
                    prog = st.progress(0)
                    browser_dir = Path.home()/".kol_tool_session"
                    pw_ctx,_pw = None,None
                    if not IS_CLOUD:
                    ...
                    for i, (_,row) in enumerate(f5_valid.iterrows()):
                        ...
                        pid = add_paid_post(sel_cid, name, plat, url,
                        ...
                        if raw["success"]:
                            update_paid_post_metrics(pid, raw.get("avg_views") or 0,
        ...
        with f5_tab2:
            ...
            if not camp_list:
                ...
                rc_name = st.selectbox("캠페인", ["전체"]+[c["campaign_name"] for c in camp_list], key="f5_rpt")
                posts = get_paid_posts(next((c["id"] for c in camp_list if c["campaign_name"]==rc_name), None) if rc_name!="전체" else None)
                if not posts:
                    st.info("포스팅이 없습니다.")
                else:
                    pdf = pd.DataFrame(posts)
                    mc = st.columns(4)
                    with mc[0]: st.metric("총 비용", f"¥{int(pdf['fee'].sum() or 0):,}")
                    with mc[1]: st.metric("총 조회수", f"{int(pdf['views'].sum() or 0):,}")
                    with mc[2]:
                    ...
                    disp_cols = ["kol_name","platform","post_date","content_type","fee","views","likes","comments","saves","cpv","cpe","er_pct"]
                    ...
                    dp = pdf[disp_cols].rename(columns={
                        ...
                        "content_type":"소재유형","fee":"비용","views":"조회수","likes":"좋아요",
                        "comments":"댓글","saves":"저장","cpv":"CPV","cpe":"CPE","er_pct":"ER%"
                    })
                    ...
                    if not cpv_data.empty:
                        ...
                        st.bar_chart(cpv_data.rename(columns={"kol_name":"KOL명","cpv":"CPV"}).set_index("KOL명"), y="CPV")
                    ...
                    if "content_type" in pdf.columns:
                        ...
                        if not tdata.empty:
                            ...
                            ts = tdata.groupby("content_type").agg(
                                포스팅수=("id","count"), 평균CPV=("cpv","mean"),
                                평균ER=("er_pct","mean"), 평균조회수=("views","mean"),
                            ).round(2).reset_index().rename(columns={"content_type":"소재유형"})
                            ts["효율점수"] = ts.apply(
                                lambda r: round(r["평균ER"]/r["평균CPV"],4) if r["평균CPV"]>0 else None, axis=1
                            )
                            ...
                            if best is not None:
                                st.markdown(f"💡 **추천 소재**: **{best['소재유형']}** (평균 CPV ¥{best['평균CPV']:.2f}, ER {best['평균ER']:.2f}%)")
    ...
    with tool_tabs[3]:
        ...
        with f6c1:
            f6_kw = st.text_input("Google Trends 키워드", placeholder="예: ANESSA 日焼け止め", key="f6_kw")
        with f6c2:
            f6_cl = get_campaigns()
            f6_cn = st.selectbox("캠페인", ["전체"]+[c["campaign_name"] for c in f6_cl], key="f6_cn")
        ...
        with f6c4:
            f6_tf = st.selectbox("분석 기간", ["최근 1개월","최근 2개월","최근 3개월"], index=1, key="f6_tf")
            ...
            if not daily:
                ...
                if f6_kw.strip():
                    try:
                        ...
                        tf_m = {"최근 1개월":"today 1-m","최근 2개월":"today 3-m","최근 3개월":"today 3-m"}
                        pt.build_payload([f6_kw.strip()], timeframe=tf_m.get(f6_tf,"today 3-m"), geo="JP")
                ...
                query_df = trends_df if trends_df is not None and not trends_df.empty else manual_df

                chart_imp = imp_df[["post_date","daily_views"]].rename(columns={"post_date":"date","daily_views":"노출수"})
                ...
                if query_df is not None and not query_df.empty:
                    ...
                    merged = pd.merge(chart_imp, query_df, on="date", how="outer").sort_values("date").fillna(0)
                    ...
                    if len(valid)>=5:
                        ...
                        for lag in range(8):
                            ...
                            if len(v2)>=3:
                                lag_r.append({"시차(일)":lag, "상관계수":round(v2["노출수"].corr(v2["sq"]),3)})
                        if lag_r:
                            st.dataframe(pd.DataFrame(lag_r), use_container_width=True, hide_index=True)
                            bl = max(lag_r, key=lambda x:abs(x["상관계수"]))
                            if bl["시차(일)"]>0:
                                st.markdown(f"→ 노출 후 약 **{bl['시차(일)']}일** 뒤 검색량 변화 최대")
                else:
    ...
    with tool_tabs[4]:
        ...
        if stats["kol_count"] == 0:
            ...
            sc = st.columns(3)
            with sc[0]: st.metric("등록 KOL", f"{stats['kol_count']}명")
            with sc[1]: st.metric("스냅샷", f"{stats['snapshot_count']}건")
            with sc[2]:
            ...
            with fc3:
                sq = st.text_input("검색", key="db_sq")

            filtered = kdf.copy()
            if pf: filtered = filtered[filtered["platform"].isin(pf)]
            if sf: filtered = filtered[filtered["campaign_status"].isin(sf)]
            if sq: filtered = filtered[filtered["name"].str.contains(sq, case=False, na=False)]
            ...
            if filtered.empty:
                ...
                show = ["KOL명","플랫폼","상태","점수","등급","ER%","평균 조회수","최초 등록","마지막 업데이트","메모"]
                show = [c for c in show if c in disp.columns]
                st.dataframe(disp[show].sort_values("점수", ascending=False, na_position="last"),
                             use_container_width=True, hide_index=True, height=300)

                kol_names = filtered["name"].tolist()
                sel_kol = st.selectbox("KOL 상세", ["선택하세요"]+kol_names, key="db_detail")
                if sel_kol != "선택하세요":
                    kr = filtered[filtered["name"]==sel_kol].iloc[0]
                    kid = int(kr["id"])
                    dc1,dc2 = st.columns([2,1])
                    with dc1:
                        st.markdown(f"**{sel_kol}** ({kr['platform']})")
                        ...
                        if delta:
                            ...
                            if items:
                                st.caption(f"변화: {' | '.join(items)}")
                        hist = get_kol_history(kid)
                        if hist:
                            hdf = pd.DataFrame(hist)
                            st.dataframe(
                                hdf[["snapshot_at","score","grade","er_pct","avg_views","cpv","fee"]].rename(columns={
                    ...
                    with dc2:
                        ...
                        new_st = st.selectbox("상태 변경", CAMPAIGN_STATUSES,
                                              index=CAMPAIGN_STATUSES.index(cur_st) if cur_st in CAMPAIGN_STATUSES else 0,
                                              key=f"st_{kid}")
                        if new_st != cur_st and st.button("저장", key=f"ss_{kid}"):
                            update_campaign_status(kid, new_st)
                            st.success("✅ 저장됨"); st.rerun()
                        cur_memo = kr.get("memo","") or ""
                        new_memo = st.text_area("메모", value=cur_memo, key=f"mm_{kid}", height=60)
                        if new_memo != cur_memo and st.button("메모 저장", key=f"sm_{kid}"):
